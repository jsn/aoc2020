...
(defn- lengths [rules id]
  (let [r (rules id)]
    (if (string? r)
      (let [c (count r)] [c c])
      (let [variants (map (fn [nodes]
                            (let [nodes (map #(lengths rules %) nodes)]
                              [(apply + (map first nodes))
                               (apply + (map second nodes))]))
                          r)]
        [(apply min (map first variants))
         (apply max (map second variants))]))))

(defn two [s]
  (let [[rules msgs] (parse s)

        _ (have! (= '([8 11]) (rules 0)))
        _ (have! #(apply = (lengths rules %)) 42 31)
